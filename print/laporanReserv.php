...
class PDF extends FPDF
{
    ...
    function Footer()
    {
        ...
        $this->Cell(550, 4, 'Banjarmasin, ' . date('d F Y'), 0, 1, 'C');
        $this->Cell(550, 4, 'PT. ANGKASA PURA SUPPORT', 0, 1, 'C'); // Mengubah tinggi menjadi 0
        $this->Cell(550, 4, 'BRANCH MANAJER', 0, 1, 'C'); // Mengubah tinggi menjadi 0
        $this->Ln(10); // Mengubah spasi 

        // Nama dan Jabatan
        $this->SetY(-60);
        $this->SetFont('Arial', 'U', 10);
        $this->Cell(550, 4, 'ALEXANDER DENALOVA', 0, 1, 'C');

        // Tanggal Timestamp
        $this->SetFont('Arial', '', 8);
        $this->Cell(550, 4, 'Tanggal: ' . date('d F Y H:i:s'), 0, 1, 'C'); // Menampilkan tanggal timestamp
...
// Create PDF instance
$pdf = new PDF('P', 'mm', array(350, 350));
$pdf->AddPage();
...
$pdf->Cell(20, 10, 'Devisi', 1);
$pdf->Cell(15, 10, 'Jabatan', 1);
$pdf->Cell(25, 10, 'Tujuan', 1);
$pdf->Cell(20, 10, 'Pilih Reser', 1);
$pdf->Cell(20, 10, 'Plat no', 1);
$pdf->Cell(20, 10, 'Merek', 1);
$pdf->Cell(20, 10, 'Tipe', 1);
$pdf->Cell(15, 10, 'Warna', 1);
...
join mobil on reserv.Plat_nomer=mobil.id where id_reserv=$id_reserv";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $pdf->Cell(20, 10, $row['Devisi'], 1);
        $pdf->Cell(15, 10, $row['Jabatan'], 1);
        $pdf->Cell(25, 10, $row['Tujuan'], 1);
        $pdf->Cell(20, 10, $row['Pilih_Reserv'], 1);
        $pdf->Cell(20, 10, $row['Plat_nomer'], 1);
        $pdf->Cell(20, 10, $row['Merek'], 1);
        $pdf->Cell(20, 10, $row['Tipe_Mobil'], 1);
        $pdf->Cell(15, 10, $row['Warna'], 1);
